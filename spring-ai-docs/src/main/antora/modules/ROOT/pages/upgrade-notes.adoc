...
=== Removed: spring-ai-azure-openai Module
...
This module was built on the Azure OpenAI SDK which is being replaced by OpenAI's official `openai-java` SDK. The `spring-ai-openai` module already supports Azure OpenAI endpoints through the `endpoint` property and `AzureIdentityCredentialProvider`, providing equivalent functionality.

Users of the removed modules should:

1. Migrate to the `spring-ai-openai` module — it supports Azure OpenAI endpoints via `spring.ai.openai.base-url` and `spring.ai.openai.api-key`
2. Use `spring.ai.openai.azure` properties for Azure-specific configuration (e.g., `spring.ai.openai.azure.chat.options.deploymentName`)
3. Replace `spring-ai-starter-model-azure-openai` with `spring-ai-starter-model-openai`


The integration tests from the removed module have been preserved in `spring-ai-azure-integration-tests` for users who need to test against Azure OpenAI endpoints.
